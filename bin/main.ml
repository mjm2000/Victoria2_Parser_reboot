        ...
        (function
            ...
            |"common" as entry-> 
                ...
                List.iter (fun entry -> 
                    ...
                    |Some "cultures.txt" -> ()
                    |Some "defines.lua" -> ()
                    |Some "event_modifiers.txt" -> ()
                    |Some "goods.txt" -> ()
                    |Some "graphicalculturetype.txt" -> ()
                    |Some "ideologies.txt" -> ()
                    |Some "issues.txt" -> ()
                    |Some "national_focus.txt" -> ()
                    |Some "nationalvalue.txt" -> ()
                    |Some "on_actions.txt" -> ()
                    |Some "pop_types.txt" -> ()
                    |Some "production_types.txt" -> ()
                    |Some "rebel_types.txt" -> ()
                    |Some "religion.txt" -> ()
                    |Some "static_modifiers.txt" -> ()
                    |Some "technology.txt" -> ()
                    |Some "traits.txt" -> ()
                    |Some "triggered_modifiers.txt" -> ()
                    |Some entry -> Printf.printf "File %s not recognized\n" entry
                    |None -> ();
                    )
                refined_common;
