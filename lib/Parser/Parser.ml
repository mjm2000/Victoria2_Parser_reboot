...
and string_rh_symbol pv = match pv with
    ...
    |Decimal -> "Decimal"
    |DefinedTypeRight t -> Printf.sprintf "DefinedTypeRight:%s" t
    |Catalog_Right (v, t) -> Printf.sprintf "Catalog_Right:%s:%s" v (string_rh_symbol t)

    |SubTable v -> Printf.sprintf "SubTable:%s" v
