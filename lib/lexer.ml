...
let lexer file = 
    let rec lex_r (chrs:char list) (buf:char list) out (x,y) = 
        ...
        |'#'::rs,_->lex_r rs ['#'] out (x+1,y)
        |'\n'::rs,['#'] -> lex_r rs [] out (1,y+1) 
        |_::rs,['#'] ->   lex_r rs ['#'] out (x+1,y)
        |'='::rs,_  -> lex_r rs [] ((EQ,str,(x,y)) ::out)  (x+1,y)
        |'{'::rs, _-> lex_r rs [] ((LB,str,(x,y))::out) (x+1,y)
        |'}'::rs, _-> lex_r rs [] ((RB,str,(x,y))::out)        (x+1,y)
        |'\n'::rs,[] -> lex_r rs [] out(1,y+1)
        |'\n'::rs,_ -> lex_r rs [] (((chars_to_lexem str),str,(x,y)) :: out) (1,y+1)
        |' '::rs,[]|'\t'::rs,[] -> lex_r rs [] out (x+1,y)
        |' '::rs,_|'\t'::rs,_-> 
            lex_r rs [] ((chars_to_lexem str,str,(x,y)) :: out) (x+1,y)

        |chr::rs,buf ->  lex_r rs (chr::buf)  out (x+1,y)
        |[],[]->out 
        |[],_-> ((chars_to_lexem str),str,(x,y)) :: out
        in
        List.rev (lex_r  (read_file_str file)  [] [] (1,1))
